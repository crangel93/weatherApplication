...
var getWeather = function () {
  ...
  //getting the city value for the text entry 
  var city2 = document.querySelector("#city").value;

  var searchBox = document.querySelector(".searchedCitys");

  var divSelector = document.querySelector(".searchedCitys");
  var pElement = document.createElement("button");
  var brElement=document.createElement("br");
  pElement.appendChild(brElement);
  pElement.classList.add("historyButtons");
  var historyButtons=document.querySelectorAll(".historyButtons");
  // pElement.setAttribute("onClick", "getWeatherSearch(" + city2 + ")");
  pElement.addEventListener("click",function(){
   console.log(pElement.innerHTML);

document.querySelector("#city").value=pElement.innerHTML;




  });
  pElement.innerHTML =  city2;
  console.log(cityCount);
  for (let i = 0; i < historyButtons.length; i++) {
    if(historyButtons[i].innerHTML===city2){
historyButtons[i].style="display:none;";

    }

  }
  divSelector.appendChild(pElement);
  divSelector.appendChild(document.createElement("br"));

  //selecting the city element h1 to add a city 
  var cityDisplay = document.querySelector("#cityCurrent")
  //creating a variable d to get methods from Dat()
  var d = new Date();
  //getting current month and adding 1 due to the index 0
  var month = d.getMonth() + 1;
  //getting the current date
  var day = d.getDate();
  //getting the full year
  var year = d.getFullYear();
  //adding the text content to the city display element
